        ...
        methods: {
            ...
            drawTable: function () {
                ...
                self.$dataTable.on('click', '[data-handle="operate-edit"]', function (e) {
                    var id = $(this).data('id');
                    self.$http.get('/api/app/roles/' + id + '/get').then(function (response) {
                ...
                self.$dataTable.on('click', '[data-handle="operate-delete"]', function (e) {
                    ...
                    swal({
                        ...
                        self.$http.delete('/api/app/roles/' + id + '/del').then(function (res) {
                            if (res.data.code == 1) {
                ...
                submit: function () {
                    ...
                    self.$validate(true, function () {
                        if (self.$validation.valid) {
                            self.disabled = true;
                            self.$http.post('/api/app/roles', self.role).then(function (response) {
                                var res = response.data;
                                if (res.code == 1) {
        ...
        var vueModal = new Vue({
                ...
                methods: {
                    ...
                    getRolePermission: function (id) {
                        var self = this;
                        self.$http.get('/api/app/roles/' + id + '/permissions?appId=' + vueIndex.appId).then(function (response) {
                            var res = response.data;
                            if (res.code == 1) {
                    ...
                    submit: function () {
                        ...
                        for (var key in self.selectedPermissions) {
                            if (self.selectedPermissions[key]) {
                                permissionIds.push(self.selectedPermissions[key]);
                            }
                        }
                        ...
                        self.$http.post('/api/app/roles/' + id + '/setPermission', {permissionIds: permissionIds}, {emulateJSON: true}).then(function (res) {
                            if (res.data.code == 1) {
                                self.$toastr.success('操作成功');
                                $el.modal('hide');
                                self.$destroy();
